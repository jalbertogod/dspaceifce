...
public class UploadStep extends AbstractProcessingStep
{
            ...
            AuthorizeException
    {
        ...
        if (buttonPressed.equals(NEXT_BUTTON) && itemService.hasUploadedFiles(item))
        {
            return STATUS_COMPLETE;
        }
        ...
        */
        ...
        int year = Util.getIntParameter(request, "dateBitstream_year");

        if (year > 0)
        {
            // save this file description
            int status = processSaveFileDateBitstream(context, request, response,
                    subInfo);
        ...
        String filePSF = request.getParameter("platformSpecificFeatures");

        if (filePSF != null && filePSF.length() > 0)
        {
            // save this file description
            int status = processSaveFilePSF(context, request, response,
                    subInfo);
        ...
        String filePlatformType = request.getParameter("platformType");

        if (filePlatformType != null && filePlatformType.length() > 0)
        {
            // save this file description
            int status = processSaveFilePlatformType(context, request, response,
                    subInfo);
        ...
        String fileLRT = request.getParameter("learningResourceType");

        if (fileLRT != null && fileLRT.length() > 0)
        {
            // save this file description
            int status = processSaveFileLRT(context, request, response,
                    subInfo);
        ...
        {
            // save this file description
            int status = processSaveFileVersion(context, request, response,
                    subInfo);
            ...
            AuthorizeException
    {
        ...
        while(attNames.hasMoreElements())
        {
            ...
            if(attr.endsWith("-path"))
            {
                ...
                //attempt to get metadata from attribute first, then direct from a parameter
                ...
                String fileDateBitstream = "";

                String dateMetadataField = "dateBitstream";

                Integer year = Util.getIntParameter(request, dateMetadataField + "_year");
                Integer month = Util.getIntParameter(request, dateMetadataField + "_month");
                Integer day = Util.getIntParameter(request, dateMetadataField + "_day");
                
                if(year>0)
                {
                    // FIXME: Probably should be some more validation
                    // Make a standard format date
                    DCDate date = new DCDate(year, month, day, -1, -1, -1);
                    fileDateBitstream = date.toString();
                }
                
                String filePSF =  (String) request.getAttribute(param + "-platformSpecificFeatures");
                if(filePSF!=null && filePSF.length()>0)
                {
                    filePSF = request.getParameter("platformSpecificFeatures");
                }

                String filePlatformType =  (String) request.getAttribute(param + "-platformType");
                if(filePlatformType!=null && filePlatformType.length()>0)
                {
                    filePlatformType = request.getParameter("platformType");
                }
                
                String fileLRT =  (String) request.getAttribute(param + "-learningResourceType");
                if(fileLRT!=null && fileLRT.length()>0)
                {
                    fileLRT = request.getParameter("learningResourceType");
                }

                String version =  (String) request.getAttribute(param + "-version");
                ...
                b.setDateBitstream(context, fileDateBitstream);
                b.setVersion(context, version);
                
                b.setPSF(context, filePSF);
                b.setPlatformType(context, filePlatformType);
                b.setLRT(context, fileLRT);
    ...
    */
    ...
    protected int processSaveFileDateBitstream(Context context,
            HttpServletRequest request, HttpServletResponse response,
            SubmissionInfo subInfo) throws ServletException, IOException,
            SQLException, AuthorizeException
    {
        if (subInfo.getBitstream() != null)
        {
            ...
            int year = Util.getIntParameter(request, dateMetadataField + "_year");
            int month = Util.getIntParameter(request, dateMetadataField + "_month");
            int day = Util.getIntParameter(request, dateMetadataField + "_day");
            ...
            if(year>0)
            {
                ...
                // Make a standard format date
                DCDate date = new DCDate(year, month, day, -1, -1, -1);
                subInfo.getBitstream().setDateBitstream(context, date.toString());
            }
    ...
    protected int processSaveFilePSF(Context context,
            HttpServletRequest request, HttpServletResponse response,
            SubmissionInfo subInfo) throws ServletException, IOException,
            SQLException, AuthorizeException
    {
        if (subInfo.getBitstream() != null)
        {
            subInfo.getBitstream().setPSF(context, 
                    request.getParameter("platformSpecificFeatures"));
    ...
    protected int processSaveFilePlatformType(Context context,
            HttpServletRequest request, HttpServletResponse response,
            SubmissionInfo subInfo) throws ServletException, IOException,
            SQLException, AuthorizeException
    {
        if (subInfo.getBitstream() != null)
        {
            subInfo.getBitstream().setPlatformType(context, 
                    request.getParameter("platformType"));
    ...
    protected int processSaveFileLRT(Context context,
            HttpServletRequest request, HttpServletResponse response,
            SubmissionInfo subInfo) throws ServletException, IOException,
            SQLException, AuthorizeException
    {
        if (subInfo.getBitstream() != null)
        {
            subInfo.getBitstream().setLRT(context, 
                    request.getParameter("learningResourceType"));
    ...
    protected int processSaveFileVersion(Context context,
            HttpServletRequest request, HttpServletResponse response,
            SubmissionInfo subInfo) throws ServletException, IOException,
            SQLException, AuthorizeException
    {
        if (subInfo.getBitstream() != null)
        {
            subInfo.getBitstream().setVersion(context, 
