...
public class UploadStep extends AbstractSubmissionStep
{
            ...
            UIException, SQLException, IOException, AuthorizeException
    {
        ...
        if (!disableFileEditing)
        {
            ...
            String fieldName = "dateBitstream";
            Composite fullDate = upload.addItem().addComposite(fieldName, "date-bitstream");
            ...
            fullDate.setLabel("Data do arquivo");
            fullDate.setHelp("Data da produção do objeto. O dia e/ou mês podem ser deixados em branco caso não sejam aplicáveis.");
            ...
            Select descriptionPSF = upload.addItem().addSelect("platformSpecificFeatures");
            descriptionPSF.setLabel("Características para uma plataforma específica");
            descriptionPSF.setHelp("Usar esta categoria para as diversas instâncias do material");   
            descriptionPSF.addOption("", "N/A");
            descriptionPSF.addOption("Versão Impressão/Gráfica", "Versão Impressão/Gráfica");
            descriptionPSF.addOption("Versão Mobile", "Versão Mobile");
            descriptionPSF.addOption("Versão Edição", "Versão Edição");

            Select descriptionPT = upload.addItem().addSelect("platformType");
            descriptionPT.setLabel("Tipo de Plataforma/Finalidade");
            descriptionPT.addOption("", "N/A");
            descriptionPT.addOption("Web", "Web");
            descriptionPT.addOption("Dispositivo Móvel", "Dispositivo Móvel");
            descriptionPT.addOption("TV Digital", "TV Digital");
            descriptionPT.addOption("Ambiente Virtual", "Ambiente Virtual");
            descriptionPT.addOption("Impressão Pessoal", "Impressão Pessoal");
            descriptionPT.addOption("Impressão em Gráfica", "Impressão em Gráfica");
            descriptionPT.addOption("Mídia Física (imagem do dvd)", "Mídia Física (imagem do dvd)");
            descriptionPT.addOption("Versão Editável", "Versão Editável");
            
            Select descriptionLRT = upload.addItem().addSelect("learningResourceType");
            descriptionLRT.setLabel("Tipo de recurso educacional");
            descriptionLRT.setHelp("Natureza ou gênero do conteúdo");
            descriptionLRT.addOption("", "N/A");
            descriptionLRT.addOption("Texto", "Texto");
            descriptionLRT.addOption("Hipertexto", "Hipertexto");
            descriptionLRT.addOption("Vídeo", "Vídeo");
            descriptionLRT.addOption("Simulação", "Simulação");
            descriptionLRT.addOption("Animação", "Animação");
            descriptionLRT.addOption("Storytelling", "Storytelling");
            descriptionLRT.addOption("Caderno Temático", "Caderno Temático");
            descriptionLRT.addOption("Apostila", "Apostila");
            descriptionLRT.addOption("Livro", "Livro");
            descriptionLRT.addOption("Áudio", "Áudio");
            descriptionLRT.addOption("Apresentação", "Apresentação");
            descriptionLRT.addOption("Curso", "Curso (Moodle. MOOC, outros)");
            descriptionLRT.addOption("Aplicativo", "Aplicativo");
            descriptionLRT.addOption("Página Web", "Página Web");
            descriptionLRT.addOption("Imagem", "Imagem (foto, ilustração, gráfico)");
            descriptionLRT.addOption("Exercício", "Exercício (hotpotatoes)");
            descriptionLRT.addOption("Mídia Interativa", "Mídia Interativa (ISO DVD)");

            Text version = upload.addItem().addText("version");
            version.setLabel("Versão");
            version.setHelp("Insira a versão do arquivo");
        ...
        if (bitstreams.size() > 0 || disableFileEditing)
        {
            ...
            header.addCellContent("Data"); // dateBitstream
            header.addCellContent("Plataforma específica"); // platformSpecificFeatures 
            header.addCellContent("Plataforma/Finalidade"); // platformType
            header.addCellContent("Recurso educacional"); // learningResourceType
            header.addCellContent("Versão"); // version
            ...
            for (Bitstream bitstream : bitstreams)
            {
                ...
                String dateBitstream = bitstream.getDateBitstream();
                String platformSpecificFeatures = bitstream.getPSF();
                String platformType = bitstream.getPlatformType();
                String learningResourceType = bitstream.getLRT();
                String version = bitstream.getVersion();
                ...
                if (dateBitstream == null || dateBitstream.length() == 0)
                {
                    row.addCellContent(T_unknown_name);
                }
                else
                {
                    row.addCellContent(dateBitstream);
                }
                if (platformSpecificFeatures == null || platformSpecificFeatures.length() == 0)
                {
                    row.addCellContent(T_unknown_name);
                }
                else
                {
                    row.addCellContent(platformSpecificFeatures);
                }
                ...
                {
                    row.addCellContent(platformType);
                }
                if (learningResourceType == null || learningResourceType.length() == 0)
                {
                    row.addCellContent(T_unknown_name);
                }
                else
                {
                    row.addCellContent(learningResourceType);
                }
